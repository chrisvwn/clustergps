  ...
  tryCatch(
    {
      ...
      else
      {
        ...
        clustCounts <- data.frame(id, day, gp, clustCounts, stringsAsFactors = F)
      }
      ...
      return(
        data.frame(id=id, day=day, gp=gp, latitude=NA, longitude=NA, conf = NA, stringsAsFactors = F)
      )
    })
}